...
% Radio base del árbol
radio_inicial = 6;

% Radio mínimo (en la punta)
radio_final = 0.4;

% Número de vueltas de la hélice
vueltas = 6;

% Vector de progresión
t = linspace(0,1,N)';

% Radio decrece suavemente (cono)
radio = radio_inicial + (radio_final - radio_inicial)*t;

% Ángulo helicoidal
theta = 2*pi*vueltas*t;

% Trayectoria base (helicoidal cónica)
xr = radio .* cos(theta);
yr = radio .* sin(theta);
zr = altura * t;
...
Px = x_c + xr;
Py = y_c + yr;
Pz = z_c - altura/2 + zr;  % acomodado para que empiece más abajo
